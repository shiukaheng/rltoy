...
def main(config: Config) -> None:
    ...
    if config.env == "frozen_lake":
        ...
        renderer = GraphRenderer(env, caption="GraphWorld SARSA(lambda) (training)")
    ...
    try:
        for episode in range(config.n_episodes):
            ...
            while not (terminated or truncated):
                ...
                pygame.display.set_caption(
                    f"SARSA(lambda={config.trace_decay}) - "
        ...
        pygame.display.set_caption("SARSA(lambda) (evaluation)")
        obs, _ = env.reset()
        renderer.set_values(np.max(Q, axis=1))
        renderer.render(obs)
